...
static void timer_10s_callback(struct timerqueue_t *tq, void *cookie)
{
	putchar('!');
	fflush(stdout);

	/* And reschedule */
	timer_queue_add(tq, 10000, timer_10s_callback, cookie);
}
...
int main(int argc, char *argv[])
{
	...
	timer_queue_add(tq, 10000, timer_10s_callback, "10s timer");
