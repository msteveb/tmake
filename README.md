...
-hash
	...
	not targets. If this is set on a rule, hashes are compute for all dependencies,
	even if those are targets.
...
* It would be expensive to compute hashes of possibly large generated targets. And generated files
  ...
  If it is desired to use hashing on certain targets, use -hash on a rule to force hashes to be used
  for its dependents rather than mtime.
