...
Now when deciding whether to build a, we check calculate hashes of all it's dependencies.
...
* If the targets need to be built for for some other reason (e.g. does not exist), the
  hash of dependents is not calculated immediately. Rather the check is done after
  all the dependents are successfully created.

* Rather than calculate the the hash every time, we store the mtime and hash of dependencies
  in the cache. If the mtime matches what is stored in the cache, the hash is used from the cache.
  Otherwise the hash is calculated and the cache entry is updated.

* It would be expensive to calculate hashes of possibly large generated targets. And generated files
...
Hashes are calculated by running 'md5sum' (or md5, for MacOS) because it is a fast hash
