...
* PublishIncludes uses "Publish --hash" to add -hash to the publish rules. Since the published
  ...
  comparison. But these published headers are really source (unless generated), so we use hash-based
  comparison to prevent unecessary rebuilds.
...
* Switching from hash-based to time-based and back to hash-based builds can cause problems with
